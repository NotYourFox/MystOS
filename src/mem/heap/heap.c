...
static uint32_t heap_align_value_to_upper (uint32_t val){
    ...
    val = (val - (val % MYSTOS_HEAP_BLOCK_SIZE));
    val += MYSTOS_HEAP_BLOCK_SIZE;
    return val;
}
...
void* heap_block_to_address(struct heap* heap, int block){
    return heap -> saddr + (block * MYSTOS_HEAP_BLOCK_SIZE);
}
...
int heap_address_to_block(struct heap* heap, void* address){
    return ((int)(address - heap -> saddr) / MYSTOS_HEAP_BLOCK_SIZE);
}

void* heap_malloc(struct heap* heap, size_t size){
    size_t aligned_size = heap_align_value_to_upper(size);
    uint32_t total_blocks = aligned_size / MYSTOS_HEAP_BLOCK_SIZE;
